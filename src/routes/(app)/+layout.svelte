<script>
	...
	// This is the app's layout, not the root one: every route a visitor would
	// recognise lives under the `(app)` group. The sandboxed widget target
	// (src/routes/embed-frame) sits outside it, under the near-empty root
	// layout, so a member's iframe neither downloads this chrome and its
	// stylesheet nor runs these stores against the opaque origin its sandbox
	// forces (where localStorage cannot persist anything).
	$effect(() => preferencesStore.init());

	$effect(() => {
		skinStore.init();
	});

	// One fetch for the whole app, started here so it is already in flight
	// whichever route the visitor landed on. Repeat calls are no-ops.
	$effect(() => {
	...
	// Ambient View (with its action panel, playlist and option sheets) is the
	// largest thing this layout renders, and most visits never open it. It is
	// imported on its own chunk: fetched once the page is idle, so the first
	// click still finds it ready, or immediately if Ambient opens before that.
	// Once loaded it stays mounted and is driven by `open`, exactly as when it
	// was a static import; everything it does is gated on `open`.
	/** @type {typeof import('../../components/AmbientView.svelte').default | null} */
	let AmbientView = $state(null);
	/** @type {Promise<void> | null} */
	let ambientViewLoading = null;

	function loadAmbientView() {
		ambientViewLoading ??= import('../../components/AmbientView.svelte').then(
			(module) => {
				AmbientView = module.default;
			},
			() => {
				// A failed chunk (a deploy replaced it mid-session) must not stay
				// cached as failed: the next open tries again.
				ambientViewLoading = null;
			}
		);
		return ambientViewLoading;
	}

	$effect(() => {
		if (ambientOpen) loadAmbientView();
	});
...
<div class="app-shell">
	...
	<!-- Development only (the panel checks the same flag itself). Gating the
	     mount here as well lets the production build drop the panel and its
	     graph from the bundle instead of shipping code that never renders. -->
