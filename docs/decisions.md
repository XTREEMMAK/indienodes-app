...
**Amended 2026-09-14: a refusing host's track plays on a second, never-wired element.** The `HEAD` probe was later replaced by trying CORS mode and reloading without it on a media error, and that reload went into the _same_ element. Once any track had wired that element (`createMediaElementSource` claims an element permanently), a refusing host reloaded into it played the zeros described above; the analysis effect could also wire the element while a refused CORS load was still pending. Reported live against `pages.kjnet.us` tracks as "MediaElementAudioSource outputs zeroes due to CORS access restrictions". `AudioPlayer` now keeps two main elements: `audioEl`, the only one ever wired, for hosts that allow CORS, and `plainEl` for hosts learned to refuse. Media events from whichever element is not holding the current track are ignored, since releasing it fires its own `pause`. `testing/audio-cors-fallback.e2e.js` reproduces the warning against a real non-CORS server and fails on the single-element player. The reactive background is still only available from hosts that send the header, so `/join` now says which hosts do, gives self-hosters the header setup, and offers "Check this track", which asks the browser the same question the player will (`$lib/audioCorsCheck.js`).
